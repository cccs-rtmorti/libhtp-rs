...
unsafe extern "C" fn LzInWindow_Create(
    ...
    let mut blockSize: UInt32 = (*p)
        .keepSizeBefore
        .wrapping_add((*p).keepSizeAfter)
        .wrapping_add(keepSizeReserv);
    if (*p).directInput != 0 {
    ...
    if (*p).bufferBase.is_null() || (*p).blockSize != blockSize {
        ...
        (*p).blockSize = blockSize;
        (*p).bufferBase =
            (*alloc).Alloc.expect("non-null function pointer")(alloc, blockSize as size_t)
                as *mut Byte
    }
...
unsafe extern "C" fn MatchFinder_ReadBlock(mut p: *mut CMatchFinder) {
    ...
    if (*p).directInput != 0 {
        let mut curSize: UInt32 =
            (0xffffffff as libc::c_uint).wrapping_sub((*p).streamPos.wrapping_sub((*p).pos));
        if curSize as libc::c_ulong > (*p).directInputRem {
            curSize = (*p).directInputRem as UInt32
        }
        (*p).directInputRem = ((*p).directInputRem as libc::c_ulong)
            .wrapping_sub(curSize as libc::c_ulong) as size_t
            as size_t;
        (*p).streamPos = ((*p).streamPos as libc::c_uint).wrapping_add(curSize) as UInt32 as UInt32;
    ...
    loop {
        let mut dest: *mut Byte = (*p)
            .buffer
            .offset((*p).streamPos.wrapping_sub((*p).pos) as isize);
        let mut size: size_t = (*p)
            .bufferBase
            .offset((*p).blockSize as isize)
            .wrapping_offset_from(dest) as libc::c_long as size_t;
        if size == 0 as libc::c_int as libc::c_ulong {
            return;
        }
        (*p).result = (*(*p).stream).Read.expect("non-null function pointer")(
            (*p).stream,
            dest as *mut libc::c_void,
            &mut size,
        );
        if (*p).result != 0 as libc::c_int {
            return;
        }
        if size == 0 as libc::c_int as libc::c_ulong {
            (*p).streamEndWasReached = 1 as libc::c_int as Byte;
        ...
        (*p).streamPos =
            ((*p).streamPos as libc::c_uint).wrapping_add(size as UInt32) as UInt32 as UInt32;
...
pub unsafe extern "C" fn MatchFinder_MoveBlock(mut p: *mut CMatchFinder) {
    memmove(
        ...
        ((*p).streamPos.wrapping_sub((*p).pos) as size_t)
            .wrapping_add((*p).keepSizeBefore as libc::c_ulong),
    );
            ...
            r = r >> 1 as libc::c_int
                ^ 0xedb88320 as libc::c_uint
                    & (0 as libc::c_int as UInt32)
                        .wrapping_sub(r & 1 as libc::c_int as libc::c_uint);
            j = j.wrapping_add(1)
        }
        (*p).crc[i as usize] = r;
        i = i.wrapping_add(1)
...
pub unsafe extern "C" fn MatchFinder_Create(
    ...
    sizeReserv = (sizeReserv as libc::c_uint).wrapping_add(
        keepAddBufferBefore
            .wrapping_add(matchMaxLen)
            .wrapping_add(keepAddBufferAfter)
            .wrapping_div(2 as libc::c_int as libc::c_uint)
            .wrapping_add(((1 as libc::c_int) << 19 as libc::c_int) as libc::c_uint),
    ) as UInt32 as UInt32;
    (*p).keepSizeBefore = historySize
        .wrapping_add(keepAddBufferBefore)
    ...
    if LzInWindow_Create(p, sizeReserv, alloc) != 0 {
        let mut newCyclicBufferSize: UInt32 =
            historySize.wrapping_add(1 as libc::c_int as libc::c_uint); /* don't change it! It's required for Deflate */
        let mut hs: UInt32 = 0;
        (*p).matchMaxLen = matchMaxLen;
        (*p).fixedHashSize = 0 as libc::c_int as UInt32;
        if (*p).numHashBytes == 2 as libc::c_int as libc::c_uint {
            hs = (((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int) as UInt32
        } else {
            hs = historySize;
            ...
            if hs > ((1 as libc::c_int) << 24 as libc::c_int) as libc::c_uint {
                if (*p).numHashBytes == 3 as libc::c_int as libc::c_uint {
                    hs = (((1 as libc::c_int) << 24 as libc::c_int) - 1 as libc::c_int) as UInt32
                } else {
                    hs >>= 1 as libc::c_int
                }
        ...
        if (*p).numHashBytes > 2 as libc::c_int as libc::c_uint {
            (*p).fixedHashSize = ((*p).fixedHashSize as libc::c_uint)
                .wrapping_add(((1 as libc::c_int) << 10 as libc::c_int) as libc::c_uint)
                as UInt32 as UInt32
        }
        if (*p).numHashBytes > 3 as libc::c_int as libc::c_uint {
            (*p).fixedHashSize = ((*p).fixedHashSize as libc::c_uint)
                .wrapping_add(((1 as libc::c_int) << 16 as libc::c_int) as libc::c_uint)
                as UInt32 as UInt32
        }
        if (*p).numHashBytes > 4 as libc::c_int as libc::c_uint {
            (*p).fixedHashSize = ((*p).fixedHashSize as libc::c_uint)
                .wrapping_add(((1 as libc::c_int) << 20 as libc::c_int) as libc::c_uint)
                as UInt32 as UInt32
        }
        hs = (hs as libc::c_uint).wrapping_add((*p).fixedHashSize) as UInt32 as UInt32;
...
unsafe extern "C" fn MatchFinder_SetLimits(mut p: *mut CMatchFinder) {
    ...
    if limit2 <= (*p).keepSizeAfter {
        ...
        limit2 = (limit2 as libc::c_uint).wrapping_sub((*p).keepSizeAfter) as UInt32 as UInt32
    }
    if limit2 < limit {
        limit = limit2
    }
...
pub unsafe extern "C" fn MatchFinder_Init_HighHash(mut p: *mut CMatchFinder) {
    ...
    let mut numItems: size_t =
        ((*p).hashMask as size_t).wrapping_add(1 as libc::c_int as libc::c_ulong);
    i = 0 as libc::c_int as size_t;
    while i < numItems {
        *items.offset(i as isize) = 0 as libc::c_int as CLzRef;
        i = i.wrapping_add(1)
...
pub unsafe extern "C" fn MatchFinder_Normalize3(
    ...
    while i < numItems {
        ...
        if value <= subValue {
            ...
            value = (value as libc::c_uint).wrapping_sub(subValue) as UInt32 as UInt32
        }
...
unsafe extern "C" fn MatchFinder_CheckLimits(mut p: *mut CMatchFinder) {
    ...
    if (*p).streamEndWasReached == 0 && (*p).keepSizeAfter == (*p).streamPos.wrapping_sub((*p).pos)
    {
...
unsafe extern "C" fn Hc_GetMatchesSpec(
    ...
    *son.offset(_cyclicBufferPos as isize) = curMatch;
    loop {
        ...
        curMatch = *son.offset(_cyclicBufferPos.wrapping_sub(delta).wrapping_add(
        ...
        if *cur.offset(maxLen as isize) as libc::c_int
            == *cur.offset((maxLen as libc::c_long + diff) as isize) as libc::c_int
        {
            ...
            while *c as libc::c_int == *c.offset(diff as isize) as libc::c_int {
                ...
                if c == lim {
                    *distances.offset(0 as libc::c_int as isize) =
                        lim.wrapping_offset_from(cur) as libc::c_long as UInt32;
            ...
            let mut len: libc::c_uint = c.wrapping_offset_from(cur) as libc::c_long as libc::c_uint;
            if maxLen < len {
                maxLen = len;
...
pub unsafe extern "C" fn GetMatchesSpec1(
    ...
    let mut ptr0: *mut CLzRef = son
        .offset(((_cyclicBufferPos as size_t) << 1 as libc::c_int) as isize)
        .offset(1 as libc::c_int as isize);
    let mut ptr1: *mut CLzRef =
        son.offset(((_cyclicBufferPos as size_t) << 1 as libc::c_int) as isize);
    ...
    loop {
        ...
        let fresh0 = cutValue;
        cutValue = cutValue.wrapping_sub(1);
        if fresh0 == 0 as libc::c_int as libc::c_uint || delta >= _cyclicBufferSize {
            *ptr1 = 0 as libc::c_int as CLzRef;
            *ptr0 = *ptr1;
            return distances;
        }
        let mut pair: *mut CLzRef = son.offset(
            ((_cyclicBufferPos
                .wrapping_sub(delta)
                .wrapping_add(if delta > _cyclicBufferPos {
                    _cyclicBufferSize
                } else {
                    0 as libc::c_int as libc::c_uint
                }) as size_t)
        ...
        if *pb.offset(len as isize) as libc::c_int == *cur.offset(len as isize) as libc::c_int {
            ...
            if maxLen < len {
                ...
                *fresh1 = len;
                ...
                distances = distances.offset(1);
                *fresh2 = delta.wrapping_sub(1 as libc::c_int as libc::c_uint);
                if len == lenLimit {
                    *ptr1 = pair0;
...
unsafe extern "C" fn SkipMatchesSpec(
    ...
    loop {
        ...
        let fresh3 = cutValue;
        cutValue = cutValue.wrapping_sub(1);
        if fresh3 == 0 as libc::c_int as libc::c_uint || delta >= _cyclicBufferSize {
            *ptr1 = 0 as libc::c_int as CLzRef;
            *ptr0 = *ptr1;
            return;
        }
...
unsafe extern "C" fn Bt2_MatchFinder_GetMatches(
    ...
    cur = (*p).buffer;
    hv = *cur.offset(0 as libc::c_int as isize) as libc::c_uint
        | (*cur.offset(1 as libc::c_int as isize) as UInt32) << 8 as libc::c_int;
    curMatch = *(*p).hash.offset(hv as isize);
    *(*p).hash.offset(hv as isize) = (*p).pos;
    offset = 0 as libc::c_int as libc::c_uint;
    offset = GetMatchesSpec1(
        lenLimit,
        curMatch,
        (*p).pos,
        (*p).buffer,
        ...
        (*p).cyclicBufferSize,
        (*p).cutValue,
        distances.offset(offset as isize),
        1 as libc::c_int as UInt32,
    )
    .wrapping_offset_from(distances) as libc::c_long as libc::c_uint;
    (*p).cyclicBufferPos = (*p).cyclicBufferPos.wrapping_add(1);
    (*p).buffer = (*p).buffer.offset(1);
    (*p).pos = (*p).pos.wrapping_add(1);
    if (*p).pos == (*p).posLimit {
...
pub unsafe extern "C" fn Bt3Zip_MatchFinder_GetMatches(
    ...
    cur = (*p).buffer;
    hv = ((*cur.offset(2 as libc::c_int as isize) as libc::c_uint
        | (*cur.offset(0 as libc::c_int as isize) as UInt32) << 8 as libc::c_int)
...
unsafe extern "C" fn Bt3_MatchFinder_GetMatches(
    ...
    cur = (*p).buffer;
    let mut temp: UInt32 = (*p).crc[*cur.offset(0 as libc::c_int as isize) as usize]
        ^ *cur.offset(1 as libc::c_int as isize) as libc::c_uint;
    h2 = temp & (((1 as libc::c_int) << 10 as libc::c_int) - 1 as libc::c_int) as libc::c_uint;
    hv = (temp ^ (*cur.offset(2 as libc::c_int as isize) as UInt32) << 8 as libc::c_int)
        & (*p).hashMask;
    hash = (*p).hash;
    pos = (*p).pos;
    d2 = pos.wrapping_sub(*hash.offset(h2 as isize));
    curMatch = *hash
        .offset(((1 as libc::c_int) << 10 as libc::c_int) as isize)
        .offset(hv as isize);
    *hash.offset(h2 as isize) = pos;
    *hash
        .offset(((1 as libc::c_int) << 10 as libc::c_int) as isize)
        .offset(hv as isize) = pos;
    maxLen = 2 as libc::c_int as libc::c_uint;
    offset = 0 as libc::c_int as libc::c_uint;
    if d2 < (*p).cyclicBufferSize
        && *cur.offset(-(d2 as isize)) as libc::c_int == *cur as libc::c_int
    {
        let mut diff: ptrdiff_t = 0 as libc::c_int as ptrdiff_t - d2 as libc::c_long;
...
unsafe extern "C" fn Bt4_MatchFinder_GetMatches(
    ...
    let mut temp: UInt32 = (*p).crc[*cur.offset(0 as libc::c_int as isize) as usize]
        ^ *cur.offset(1 as libc::c_int as isize) as libc::c_uint;
    h2 = temp & (((1 as libc::c_int) << 10 as libc::c_int) - 1 as libc::c_int) as libc::c_uint;
    temp ^= (*cur.offset(2 as libc::c_int as isize) as UInt32) << 8 as libc::c_int;
    h3 = temp & (((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int) as libc::c_uint;
    hv = (temp ^ (*p).crc[*cur.offset(3 as libc::c_int as isize) as usize] << 5 as libc::c_int)
        & (*p).hashMask;
    hash = (*p).hash;
    pos = (*p).pos;
    d2 = pos.wrapping_sub(*hash.offset(h2 as isize));
    ...
    curMatch = *hash
        .offset(
            (((1 as libc::c_int) << 10 as libc::c_int) + ((1 as libc::c_int) << 16 as libc::c_int))
                as isize,
        )
        .offset(hv as isize);
    *hash.offset(h2 as isize) = pos;
    ...
    *hash
    ...
    {
        maxLen = 2 as libc::c_int as libc::c_uint;
        *distances.offset(0 as libc::c_int as isize) = 2 as libc::c_int as UInt32;
        *distances.offset(1 as libc::c_int as isize) =
            d2.wrapping_sub(1 as libc::c_int as libc::c_uint);
        offset = 2 as libc::c_int as libc::c_uint
    }
    if d2 != d3
        && d3 < (*p).cyclicBufferSize
        && *cur.offset(-(d3 as isize)) as libc::c_int == *cur as libc::c_int
    {
        maxLen = 3 as libc::c_int as libc::c_uint;
        *distances
            .offset((offset as size_t).wrapping_add(1 as libc::c_int as libc::c_ulong) as isize) =
    ...
    if offset != 0 as libc::c_int as libc::c_uint {
        ...
        maxLen = c.wrapping_offset_from(cur) as libc::c_long as libc::c_uint;
        *distances
            .offset((offset as size_t).wrapping_sub(2 as libc::c_int as libc::c_ulong) as isize) =
            maxLen;
        if maxLen == lenLimit {
            SkipMatchesSpec(
                lenLimit,
...
unsafe extern "C" fn Bt2_MatchFinder_Skip(mut p: *mut CMatchFinder, mut num: UInt32) {
    loop {
        ...
        if !(num != 0 as libc::c_int as libc::c_uint) {
            break;
        }
    }
}
...
unsafe extern "C" fn Bt4_MatchFinder_Skip(mut p: *mut CMatchFinder, mut num: UInt32) {
    loop {
        ...
        if lenLimit < 4 as libc::c_int as libc::c_uint {
            ...
            hv = (temp
                ^ (*p).crc[*cur.offset(3 as libc::c_int as isize) as usize] << 5 as libc::c_int)
                & (*p).hashMask;
            hash = (*p).hash;
            curMatch = *hash
                .offset(
